...
*首页｜截图展示6.3.11的四个入口；6.3.20另增视频接续工作区。*
...
图片与视频模式共用制作参数入口、状态与计时呈现、错误反馈和候选整理方式。切换创作模式时，熟悉的操作保持一致；具体参数则按当前工具与工作流适配。
...
资产详情还可展开来源项目、生成当时的制作记录与派生关系，点击来源返回对应项目。五个模式使用共同的素材添加、管理和入库体验；固定版本与来源信息帮助下一次创作衔接已有成果。

## 提示词：在创作中积累，在不同项目中复用

在创作模块保存项目后，提示词自动收录到独立管理库，并按视频／图片／剧本及模型家族分类。H3 的不同变体保持同一家族，图片按 Krea2 归类；无需每次手动整理分类。

你可以搜索、收藏、编辑模板、保留固定版本，并选择需要的正文或声音字段应用到当前草稿。生成记录保留当时实际使用的文字，便于回看和复用；提示词库可单独备份。未来剧本创作与本地／云端 LLM 已预留接入契约，当前版本不含相应创作页面或模型调用。
...
**v6.3.32 配套工具包：[下载 ZIP](https://github.com/sayuel-tech/TimeForest/releases/download/v6.3.32/TimeForest-Companion-v6.3.32.zip) · [GitHub Release](https://github.com/sayuel-tech/TimeForest/releases/tag/v6.3.32)**
...
- `prompt_library_dir` 可配置独立提示词库；默认存放在项目数据目录内，提示词库和补记快照不提交 Git。
