...
[五种模式](#五种创作模式) · [视频制作](#视频从素材与分镜走向成片) · [图片筛选](#生成与筛选让多次尝试更顺手) · [资产复用](#资产让素材成为可复用的创作积累) · [安装与启动](README.md#安装与启动)
...
你可以搜索、收藏、编辑模板、保留固定版本，并选择需要的正文或声音字段应用到当前草稿。生成记录保留当时实际使用的文字，便于回看和复用；提示词库可单独备份。剧本与电影已有开发中的页面和接口，但两个模式目前不可用，不列入本版可用功能。
