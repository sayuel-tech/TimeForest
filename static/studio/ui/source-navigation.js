...
export function showSourceNavigation(root,target){
  ...
  const html=`<aside class="notice row between" data-source-navigation role="status"><span>${esc(target.message)}</span><div class="row">${assetVersionLink(target,'返回来源资产')}${target.recycle?`<a href="#/assets?view=trash&amp;recycle=${esc(target.recycle)}">查看对应回收站</a>`:''}<button type="button" class="quiet" data-dismiss-source>关闭提示</button></div></aside>`;
