...
export function recordControl(record,{selected=false,busy=false,image=false}={}) {
  if(record.removed_at)return `<button type="button" class="quiet" data-record-restore="${esc(record.id)}" ${busy?'disabled':''}>恢复</button>`;
  if(selected)return '<small>当前已选用，换选后可移除</small>';
  const ended=(image?['success','failed','cancelled']:['complete','failed']).includes(record.state||record.status);
  if(!ended)return '<small>运行中或待确认，暂不可移除</small>';
  return `<button type="button" class="quiet" data-record-remove="${esc(record.id)}" ${busy?'disabled':''}>移除</button>`;
}

export function removedRecords(records,{busy=false,preview=()=>''}={}) {
  const removed=records.filter(r=>r.removed_at);
  if(!removed.length)return '';
  return `<details class="removed-records"><summary>已移除（${removed.length}）</summary><p class="helper">已从候选列表移除。原文件、资产和已有引用保留，可恢复。<a href="#/assets?view=trash&recycle=generations">前往回收站查看全部</a></p>${removed.map(r=>`<div class="candidate-record row between">${preview(r)}<small>${esc(r.id.slice(0,8))} · 种子 ${esc(r.seed??'未记录')}</small>${recordControl(r,{busy})}</div>`).join('')}</details>`;
}
