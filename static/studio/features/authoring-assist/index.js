...
export function mountAssist({ctx,content,edit,row,save,run,render,controller,reload}){
  ...
  box.querySelectorAll('[data-ai-task]').forEach(b=>b.onclick=()=>run(async()=>{
    ...
    const providers=(await api('/authoring/providers')).items;const provider=providers.find(c=>['deepseek','openai_compatible'].includes(c.provider_kind)&&c.enabled)||providers.find(c=>['deepseek','openai_compatible'].includes(c.provider_kind))||providers[0];
...
async function settings(ctx,controller){
  ...
  try{
    ...
    if(data.custom_chat_version!==1)throw Error('后台尚未加载自定义云端接口，请保存项目并重启时间森林后再打开设置。');
    const original=data.items.find(p=>['deepseek','openai_compatible'].includes(p.provider_kind)&&p.enabled)||data.items.find(p=>['deepseek','openai_compatible'].includes(p.provider_kind)),draft=structuredClone(ctx.providerDraft||original);let secret='';
    const render=()=>{
      modal.content.innerHTML=productionSettingsMarkup({scope:'云端写作服务 · 适用于剧本 AI 请求',sections:[{key:'provider',title:'服务与模型',html:`<div class="settings-grid">${field('接口类型',`<select data-config="provider_kind"><option value="deepseek" ${draft.provider_kind==='deepseek'?'selected':''}>DeepSeek Chat</option><option value="openai_compatible" ${draft.provider_kind==='openai_compatible'?'selected':''}>兼容 Chat Completions</option></select>`)}${field('服务地址',`<input data-config="base_url" value="${esc(draft.base_url)}" placeholder="https://服务地址/v1">`,'可填写基础地址或完整 /chat/completions 地址；按服务商说明保留路径前缀。')}${field('模型 ID',`<input data-config="model_id" value="${esc(draft.model_id||'')}">`,'填写服务商提供的完整模型 ID；修改后用于新的写作请求，不改变历史输出。')}${field('JSON 返回方式',`<select data-config="structured_mode"><option value="json_object" ${draft.structured_mode==='json_object'?'selected':''}>JSON 模式（服务须支持）</option><option value="prompt_json" ${draft.structured_mode==='prompt_json'?'selected':''}>仅提示词要求 JSON（兼容接口）</option></select>`,'服务不支持 response_format 时选择仅提示词要求；返回仍须符合剧本格式。')}${field('图片输入能力',`<select data-config="vision"><option value="unknown" ${draft.vision==='unknown'?'selected':''}>尚未确认</option><option value="supported" ${draft.vision==='supported'?'selected':''}>已确认支持</option><option value="unsupported" ${draft.vision==='unsupported'?'selected':''}>不支持</option></select>`)}${field('最大输出 token',`<input data-config="max_output_tokens" type="number" min="1" step="1" value="${draft.max_output_tokens}">`)}<label class="field"><span class="check"><input data-enabled type="checkbox" ${draft.enabled?'checked':''}>启用云端写作</span></label></div><p class="helper">配置用于所有剧本项目的新请求。兼容接口使用 Bearer 密钥与 Chat Completions 协议；保存不会调用模型。</p>`},{key:'credential',title:'账户凭据',html:`${field('服务密钥',`<input data-secret type="password" autocomplete="new-password" placeholder="${original.credential_ref?'已配置，留空保留':'尚未配置'}">`)}<p class="helper">密钥留空保留原值；换服务商时请填写对应密钥。密钥只保存在网站后端，不进入剧本与提示词库。</p>`}],actions:productionSettingsActions([{role:'cancel',label:'取消'},{role:'apply',label:'应用'},{role:'save',label:'保存',primary:true}])});
