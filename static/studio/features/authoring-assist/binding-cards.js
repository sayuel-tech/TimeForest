...
export function mountBindingCards({ctx,content,edit,save,run,render,reload,controller},box){
  ...
  const pending=effective.filter(b=>b.state==='pending').length;
  box.innerHTML=`<div class="asset-needs-heading"><div><h2>${scope==='project'?'资产需求':scope==='shot'?'本分镜资产':'本片段资产'}</h2><details class="asset-needs-help"><summary>说明</summary><p class="helper">按需求绑定素材。更换素材保留资产原件，不自动改写剧本。</p></details></div><span class="asset-needs-progress" role="status">${pending} 项待落实</span></div>${target?field('基础来源',`<select data-asset-policy>${opts(choices,data.policies?.[id]||'parent')}</select>`):''}
  ...
  function filterNeeds(){
    ...
    box.querySelector('[data-no-needs]').hidden=visible>0||!needs.length;
  }
  box.querySelectorAll('[data-need-filter]').forEach(button=>button.onclick=()=>{view.kind=button.dataset.needFilter;filterNeeds();});
  filterNeeds();
  for(const need of needs){
    ...
    const description=card.querySelector('.asset-need-description');if(description)card.querySelector('.asset-need-options').append(description);
    ...
    if(available.length){
      ...
      card.querySelector('.asset-need-options').append(details);
  ...
  box.querySelector('[data-missing-only]')?.addEventListener('change',e=>{view.missing=e.target.checked;filterNeeds();});
