...
export function shotProperties(ctx,s) {
  ...
  const advanced=`${s.index&&!swap?field('与上一段的关系',`<select aria-label="P${s.index+1}场景边界" data-field="boundary" data-segment="${s.id}">${opts([['continue','连续承接'],['new_scene','新场景']],s.boundary)}</select>`,'只有新场景独立开始；素材继承与续接分开控制。'):''}${field('种子模式',`<select data-field="seed_mode" data-segment="${s.id}">${opts([['random','每次随机'],['fixed','固定种子']],s.seed_mode)}</select>`)}${field('固定种子',`<input data-field="seed" data-segment="${s.id}" inputmode="numeric" value="${esc(s.seed)}">`,'上次实际使用：'+(s.last_seed??'尚未生成'))}${!swap?field('正文方式',`<select data-field="prompt_mode" data-segment="${s.id}">${opts([['structured','按工作流整理结构'],['full','完整H3正文']],s.prompt_mode)}</select>`)+control(s,'staging','场景与站位')+control(s,'beats','视觉节拍')+control(s,'ending','结束状态'):''}`;
