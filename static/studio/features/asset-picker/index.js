      ...
      try {
        ...
        view.querySelector(".library-picker-results").innerHTML =
          data.items
            .map(
              (item) =>
                `<button class="library-pick-card" data-pick="${item.id}" ${exclude.includes(item.id) ? "disabled" : ""}>${thumbnail(item)}<strong>${esc(item.name)}</strong><small>${esc(mediaSummary(primaryMedia(item)))}</small></button>`,
            )
            .join("") ||
