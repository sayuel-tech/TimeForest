...
import { getMode, listModes, setImageAssetsEnabled, setAssemblyEnabled, setCreationEnabled } from "./mode-registry.js";
...
async function createProject(modeId) {
  ...
  if(['authoring','movie'].includes(modeId)){
    const {createCreationProject}=await import('../features/authoring-assist/create-project.js');
    return createCreationProject(mode,go);
  }
  const d = ui.scopedModal(
    `<div class="dialog-heading"><span class="eyebrow">${mode.code} · NEW PRODUCTION</span><h2>${mode.name}</h2><p>${mode.description}</p></div><form id="new-form">${ui.field("项目名称", '<input name="name" required maxlength="120" placeholder="给这个故事一个名字" autofocus>')}${mode.kind === "image" ? ui.field("编辑工具", '<select name="submode"><option value="single">单图编辑</option><option value="dual">多图编辑</option><option value="region">局部重绘／移除</option><option value="outpaint">图像扩展</option><option value="text">文生图</option></select>') : ""}${modeId !== "swap" && mode.kind !== "image" && mode.kind !== "assembly" ? ui.field("计划总时长（秒）", '<input name="duration" type="number" min="1" max="3600" step=".01" value="15" required>', "每15秒一个片段；30秒写两段提示词。") : ""}<p class="helper">创建后进入${mode.entry}，准备完成后再生成${mode.kind === "image" ? "图片" : "视频"}。</p><div class="dialog-actions"><button type="button" id="close-new">取消</button><button class="primary">开始创作 →</button></div></form>`,
  );
  let creating=false;
...
async function route() {
  ...
  try {
    ...
    if (match) {
      ...
      if(['authoring','movie'].includes(p.kind)){
        const {mountWorkspace}=await getMode(p.mode).load();
        if(current!==epoch)return;
        workspace=mountWorkspace(root,p);return;
      }
      if(p.kind==='assembly'){
      ...
      setCreationEnabled(data.creation_contract_version);
...
async function health() {
  try {
    ...
    setCreationEnabled(h.creation_contract_version);
