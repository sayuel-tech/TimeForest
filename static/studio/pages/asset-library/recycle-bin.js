    ...
    try{
      ...
      content.querySelectorAll('[data-recycle-restore]').forEach(button=>button.onclick=async()=>{
        ...
        try{
          ...
          if(!await ui.confirm('恢复「'+item.title+'」？',(item.task_name?item.task_name+'。':'')+'将恢复到原来的列表。生成记录不会自动选用；恢复项目不会恢复其中另行移除的生成记录。','恢复'))return;
          if(signal.aborted)return;
