...
export function renderEdit(ctx) {
  ...
  const sourceCanvas=`<div class="shot-heading"><h2>参考表演</h2><span class="helper">${state}</span></div>${sourcePlayer(ctx)}<div class="source-actions"><label class="upload">${hasSource?'替换源视频':'上传参考视频'}<input id="source-file" type="file" accept="video/*"></label>${ctx.catalog.asset_library_version?'<button type="button" data-library-source>从资产库选择</button>':''}</div>`;
  ...
  const sourcePage=workbench({kind:'source-desk',canvas:sourceCanvas,inspector:sourceProperties})+`<div class="savebar"><span id="save-state">${ctx.dirty?'有未保存修改':'已保存 · 版本 '+p.revision}</span><div class="row"><button id="save" class="quiet">保存草稿</button><button id="open-segments" class="primary" ${preparing||!hasSource?'disabled':''}>${p.source_ready?'查看分段与替换 →':'准备并继续 →'}</button></div></div>`;
