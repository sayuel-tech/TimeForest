...
export function mountWorkspace(root,project,catalog){
  ...
  function render(){
    ...
    const extensions=c?.extensions.filter(x=>!x.removed_at)||[],extensionIndex=extensions.indexOf(e);
    const previous=extensions[extensionIndex-1];
    const startLabel=previous?`${c.name} · 续写 ${extensionIndex}（${previous.selected?'已选结果':'尚未选用结果'}）`:`${c?.name||'原视频'} · 使用终点 ${ui.fmt(c?.end||0)}秒`;
    const selectedRun=runs.find(v=>v.id===e?.selected);
    const candidateIdentity=r?`正在查看：候选 ${allRuns.indexOf(r)+1} · ${selectedRun?`已选用：候选 ${allRuns.indexOf(selectedRun)+1}`:'尚未选用候选'}`:'';
    ...
    if(step===0){
      ...
      canvas=`<div class="shot-heading"><h2>${ui.esc(c?.name||'尚未导入视频')}${e?` · 续写 ${extensionIndex+1}`:''}</h2><button data-add-extension ${!c||locked?'disabled':''}>＋ 添加续接</button></div><div class="assembly-extension-tabs">${c?.extensions.filter(x=>!x.removed_at).map((item,i)=>`<button data-extension="${item.id}" aria-pressed="${item.id===e?.id}">续写 ${i+1} · ${item.selected?'已选用':'待选用'}</button>`).join('')||''}</div>`;
      if(e){
        canvas+=`<div class="row between"><p data-continuation-source>下一次接续起点：${ui.esc(startLabel)}</p><button class="text-link" data-remove-extension ${disabled}>移除续写段</button></div><p class="helper">轨道排序只改变成片顺序，不改变接续起点。修改描述或参数后，重新生成并选用才会改变成片；已选结果保留。</p><div class="row"><button data-preflight ${disabled}>检查工作流</button><button class="primary" data-generate ${disabled}>${runs.length?'重新生成':'生成续接'}</button></div>${ui.field('后续画面与声音描述',`<textarea data-prompt rows="4" ${draftDisabled}>${ui.esc(e.prompt)}</textarea>`)}${ui.field('希望新增时长（秒）',`<input data-seconds type="number" min="1" max="3600" step=".01" value="${e.seconds}" ${draftDisabled}>`,'内部会按模型合法帧数分段，交付只保留新增部分。')}${runView(r)}${candidates(r)}<p class="helper" data-candidate-identity>${candidateIdentity}</p>${player(r?.url)}${r?.state==='success'?resultActionBar({inspect:'<button data-play-tail>连播原片尾与续接</button>',decide:`<button data-select="${r.id}" ${locked||e.selected===r.id?'disabled':''}>${e.selected===r.id?'已选用':'选用此结果'}</button><button data-continue ${!e.selected||locked?'disabled':''}>沿已选结果继续续接</button>`,collect:resultActions(r)}):''}`;
      }else canvas+='<div class="empty"><h3>需要更长的镜头？</h3><p>添加续接并写下后续内容。不需要续接时，可以直接拼接导出。</p></div>';
    }else{
      canvas=`<h2>拼接成片</h2><p data-export-summary>${track.length}个轨道片段 · 共 ${ui.fmt(track.reduce((sum,item)=>sum+item.seconds,0))}秒 · 按下列顺序拼接</p><ol>${track.map(v=>`<li>${ui.esc(v.name)} · ${ui.fmt(v.seconds)}秒</li>`).join('')}</ol><p>输出 ${p().assembly.output.width} × ${p().assembly.output.height} · ${p().assembly.output.fps} fps；${p().assembly.output.fit==='contain'?'等比例补边':'填满裁切'}。普通拼接不使用ComfyUI。</p><button class="primary" data-export ${locked||!c?'disabled':''}>开始拼接</button>${runView(r)}${candidates(r)}${player(r?.url)}${r?.state==='success'?resultActionBar({collect:resultActions(r)}):''}`;
    }
