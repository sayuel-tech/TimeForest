...
import {productionGroups} from '../../ui/production-settings.js';
...
export function sourceParametersMarkup(records){
  if(!Array.isArray(records)||!records.some(r=>r.fields?.length))return '';
  return `<details class="source-production-parameters"><summary>查看原片段制作参数</summary><p class="helper">来自制作时保存的运行记录，仅供对照；未记录字段留空，不会应用到当前制作参数。已关闭或跳过的配置不代表本次执行启用。</p>${records.filter(r=>r.fields?.length).map(r=>`<section><h4>${esc(r.title)}</h4>${Object.entries(productionGroups).map(([group,label])=>{
