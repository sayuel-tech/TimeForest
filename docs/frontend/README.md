...
UI/UX 变更先读 [按任务分层索引](../uiux/README.md)。此文保留架构与技术规则，具体页面、公共调用方和美术定位由分层索引导航。
...
详细证据和备份位于 `<本地维护路径>`。参阅其中 `checks/验收记录.md` 与 `发布与回退.md`。当前维护目录与归档边界见 [维护基线](../maintenance-baseline.md)。
...
历史实施及验收见 [V3 实施记录](<本地维护路径>)。该记录的静态入口 6.2.4 是历史版本；当前版本只在维护基线登记。视频输入契约仍为 input_contract_version=2，图片项目 API 快照另有 image_contract_version=1，不将其混作单次生成运行快照字段。
...
新模式实际实现见 [剧本与电影](../creation-modes.md)。入口通过 creation_contract_version=1 显示，旧后台不误开页面。五页剧本与两页电影共用原 workspace-chrome/workbench/actions；专属业务在 modes/authoring、modes/movie。五张新美术已独立授权制作并接入，8项宽窄及缺图回退检查通过，详见V3美术证据。





字体、控件、间距与布局维护统一读[视觉尺寸规范](visual-design-standard.md)，使用design-tokens.css和公共外壳；本轮页面截图及检查在[evidence/ui-proportion-20260909](../evidence/ui-proportion-20260909/README.md)。
