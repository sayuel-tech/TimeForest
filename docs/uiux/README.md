...
[任务导向实施计划](task-led-plan.md)：各页面卡已补充三问、空间判断、保留内容和直接验收场景；按有限批次实施，唯一当前批次仍记录在维护基线。气质/行为/组件统一，空间服从业务。


[任务导向公共复核](audit/task-led-closeout.md)：Q7行为矩阵、证据范围与未验收边界；当前状态仍只见维护基线。
