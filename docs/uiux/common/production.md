...
Q4c直接联查（2026-09-12）：shotPrompt的HTML前导换行保护文生/参考图正文开头空白；workbench.css仅在窄屏带shot-heading的工作区为目录工具行保留实际高度，防止遮挡同排侧栏按钮。前三视频保留原保存和审核。文生六例、参考图与换人各四例及文生原结果两档通过，范围和证据见[text-story页面卡](../pages/text-story.md#q4c文生写作与审核2026-09-12)。Q5a补核：shot-heading规则同时影响图片结果页，已补图片宽窄检查；Q5a另将图片编辑image-tools纳入同一公共避让规则，详见image-assets页面卡。
