...
Q7公共复核（2026-09-12）：保存/取消、迟到响应、候选查看/选用/入库及恢复的跨页含义已按实际变更核对，36项公共检查通过；未改状态基础或运行代码。依据与有限边界见[行为矩阵](../audit/task-led-closeout.md)，不能替代真实生成或整体体验验收。

2026-09-12 图片多图新增core/image-inputs.js，仅组织稳定A—I槽位；保存、异步、上传和参数取消沿用原机制，无公共状态行为改动。宽窄真实API检查包含取消移除不提交与保存重开，见图片页面卡。
