...
CHECK = r'''
 ...
 await click('[data-cancel]');await click('#settings');await click('[data-settings-section="sampling"]');
 const oldSteps=Number(document.querySelector('[data-param="steps"]').value),nextSteps=oldSteps+4;
 input('[data-param="steps"]',nextSteps);await click('dialog [data-apply]');await wait(()=>!document.querySelector('#dialog').open);
 assert((await get()).assembly.clips[0].extensions[0].configurations.dance_split.steps===oldSteps,'Apply persisted unexpectedly');
 await click('#settings');assert(Number(document.querySelector('[data-param="steps"]').value)===nextSteps,'Apply lost draft');
 const originalFetch=window.fetch;let saveCount=0,releaseSave;
 window.fetch=(url,options)=>String(url).endsWith('/save')?new Promise(resolve=>{saveCount++;releaseSave=()=>resolve(new Response(JSON.stringify({error:'隔离保存冲突，草稿保留',error_kind:'conflict',error_raw:'fixture revision conflict'}),{status:409,headers:{'Content-Type':'application/json'}}));}):originalFetch(url,options);
 const saveButton=document.querySelector('dialog [data-save]');saveButton.click();saveButton.click();
 await wait(()=>releaseSave);assert(saveCount===1,'double save submitted twice');
 document.querySelector('#dialog').dispatchEvent(new Event('cancel',{cancelable:true}));assert(document.querySelector('#dialog').open,'pending save closed');
 releaseSave();await wait(()=>document.querySelector('#parameter-errors .error-feedback'));
 assert(Number(document.querySelector('[data-param="steps"]').value)===nextSteps,'failure lost input');
 window.fetch=originalFetch;await click('dialog [data-save]');await wait(()=>!document.querySelector('#dialog').open);
 assert((await get()).assembly.clips[0].extensions[0].configurations.dance_split.steps===nextSteps,'retry not persisted');
 await click('#settings');await click('[data-reset]');await click('[data-cancel]');await click('#settings');
 assert(Number(document.querySelector('[data-param="steps"]').value)===nextSteps,'cancelled reset changed draft');
 await click('[data-cancel]');await click('#settings');input('[data-param="steps"]',oldSteps);await click('dialog [data-apply]');await wait(()=>!document.querySelector('#dialog').open);
 const {confirmLeave}=await import('/static/studio/ui/choice-dialog.js');
 const decision=confirmLeave({save:()=>workspace.saveBeforeLeave()});await click('.choice-dialog [data-choice="2"]');assert(await decision==='save','leave save failed');
 assert((await get()).assembly.clips[0].extensions[0].configurations.dance_split.steps===oldSteps,'leave save not persisted');
 await click('#settings');
 const roles=[...document.querySelectorAll('[data-settings-action]')].map(el=>el.dataset.settingsAction);assert(roles.join(',')==='reset,cancel,apply,save','actions differ');
