...
try{
 ...
 await document.fonts.ready;if(q.has('settings')){const dialog=document.querySelector('#dialog'),content=document.querySelector('.production-settings');assert(content,'shared dialog root');assert(getComputedStyle(content.querySelector('h2')).fontSize===getComputedStyle(document.documentElement).getPropertyValue('--title-dialog').trim(),'shared heading style');assert(dialog.getBoundingClientRect().width>Math.min(1000,innerWidth*.8),'shared width');assert(content.querySelector('.settings-grid'),'shared field grid');}assert(document.documentElement.scrollWidth<=innerWidth+1,'horizontal overflow');
