...
try{
 ...
 const heading=root.querySelector('[data-workspace-header] h1');assert(parseFloat(getComputedStyle(heading).fontSize)===(innerWidth<=760?28:32),'page title scale');
  ...
  try:
   for p,count in items:
    ...
    for width,step,settings in scenes:
     ...
     result=subprocess.run(['C:/Program Files/Google/Chrome/Application/chrome.exe','--headless=new','--disable-gpu','--no-first-run','--disable-background-networking',f'--user-data-dir={out/("profile-"+name)}',f'--window-size={width},1000','--virtual-time-budget=10000',f'--screenshot={out/(name+".png")}','--dump-dom',url],capture_output=True,timeout=40)
     dom=result.stdout.decode('utf-8','replace');match=re.search(r'data-check="([^"]+)"',dom);data=json.loads(html.unescape(match[1])) if match else dict(passed=False,error='No completion marker')
